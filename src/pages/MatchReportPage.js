...
import api from "../api"; // your axios/fetch wrapper configured with baseURL
...
const MatchReportPage = () => {
  ...
  useEffect(() => {
    ...
    api
      .get("/tournaments", { params: { teamCategory: selectedCategoryLeft } })
      .then((res) => {
  ...
  useEffect(() => {
    ...
    api
      .get("/matches", {
        params: { teamCategory: selectedCategoryLeft, tournament: selectedTournamentLeft },
      })
      .then((res) => {
  ...
  // UPDATE TEAMS WHEN MATCH CHANGES (LEFT)
  useEffect(() => {
  ...
  // FETCH PLAYERS WHEN TEAM CHANGES (LEFT)
  useEffect(() => {
    if (!selectedTeamLeft) {
      setPlayersLeft([]);
      setSelectedPlayer(null);
    ...
    api
      .get("/team-players", { params: { team_id: selectedTeamLeft.id } })
      .then((res) => setPlayersLeft(res.data))
      .catch(console.error);
  }, [selectedTeamLeft]);
  ...
  useEffect(() => {
    ...
    api
      .get("/tournaments", { params: { teamCategory: selectedCategoryRight } })
      .then((res) => {
  ...
  useEffect(() => {
    ...
    api
      .get("/matches", {
        params: { teamCategory: selectedCategoryRight, tournament: selectedTournamentRight },
      })
      .then((res) => {
  ...
  // UPDATE TEAMS WHEN MATCH CHANGES (RIGHT)
  useEffect(() => {
  ...
  // GENERATE PLAYER REPORT PDF (OPEN IN NEW TAB)
  ...
  // GENERATE TEAM REPORT PDF (OPEN IN NEW TAB)
  const handleGenerateTeamReport = () => {
    if (!selectedMatchRight || !selectedTeamRight) return;
    const url = `${api.defaults.baseURL}/team-match-report/${selectedMatchRight}/${selectedTeamRight.id}/pdf`;
    window.open(url, "_blank");
  };
