...
const [ddMinBallsStyle, setDdMinBallsStyle] = useState(60);  // was 120
const [ddMinBallsDeath, setDdMinBallsDeath] = useState(36);  // was 60
...
const openDoDontModal = async () => {
  ...
  // Load venue options (optional) but don't block showing the modal
  try {
...
// When modal is open, (re)fetch on ground/time/threshold change
useEffect(() => {
  if (!showDoDont) return;
  fetchDoDonts({ ground: ddGround || undefined, time_of_day: ddTime || undefined });
  // eslint-disable-next-line react-hooks/exhaustive-deps
}, [showDoDont, ddGround, ddTime, ddMinBallsStyle, ddMinBallsDeath]);
                    ...
                    <Modal.Body>
                        ...
                        <Card.Body>
                            ...
                            <Col md={3} className="text-end">
                                ...
                                onClick={() => fetchDoDonts({ ground: ddGround || undefined, time_of_day: ddTime || undefined })}
                                disabled={ddLoading}
                                >
                        ...
                        <>
                            ...
                            {/* GLOBAL PRINCIPLES */}
                            {(ddData.global?.do?.length || ddData.global?.dont?.length) && (
                            <Card className="mb-3" bg={cardVariant} text={isDarkMode ? "light" : "dark"}>
                                <Card.Body>
                                <h5 className="fw-bold mb-3">Global Principles</h5>
                                <Row className="g-3">
                                    <Col md={6}>
                                    <div className="fw-bold mb-1">Do</div>
                                    <ul className="mb-0">
                                        {(ddData.global?.do || []).map((d, i) => (
                                        <li key={i}>
                                            {d.text}
                                    ...
                                    <Col md={6}>
                                    <div className="fw-bold mb-1">Don’t</div>
                                    <ul className="mb-0">
                                        {(ddData.global?.dont || []).map((d, i) => (
                                        <li key={i}>
                                            {d.text}
                                            {d.phase && <PhasePill text={d.phase} />}
                                            {d.evidence && <div className="small text-muted">{d.evidence}</div>}
                            ...
                            {/* PHASED GUIDANCE */}
                            <h5 className="fw-bold mt-3 mb-2">Phased Guidance</h5>
                            <Row className="g-3">
                            {/* Powerplay */}
                            <Col md={4}>
                                <Card bg={cardVariant} text={isDarkMode ? "light" : "dark"} className="h-100">
                                <Card.Body>
                                    <h6 className="fw-bold">Powerplay (0–6)</h6>
                                    <Row>
                                    <Col md={6}>
                                        <div className="fw-bold mb-1">Do</div>
                                        <ul className="mb-0">
                                        {(ddData.phases?.pp?.do || []).map((d, i) => (
                                            <li key={i}>
                                            {d.text}
                                            {d.evidence && <div className="small text-muted">{d.evidence}</div>}
                                            </li>
                                        ))}
                                        {!(ddData.phases?.pp?.do || []).length && <li className="text-muted">—</li>}
                                        </ul>
                                    </Col>
                                    <Col md={6}>
                                        <div className="fw-bold mb-1">Don’t</div>
                                        <ul className="mb-0">
                                        {(ddData.phases?.pp?.dont || []).map((d, i) => (
                                            <li key={i}>
                                            {d.text}
                                            {d.evidence && <div className="small text-muted">{d.evidence}</div>}
                                            </li>
                                        ))}
                                        {!(ddData.phases?.pp?.dont || []).length && <li className="text-muted">—</li>}
                                        </ul>
                                    </Col>
                                    </Row>
                                </Card.Body>
                            ...
                            <Col md={4}>
                                <Card bg={cardVariant} text={isDarkMode ? "light" : "dark"} className="h-100">
                                <Card.Body>
                                    <h6 className="fw-bold">Middle (7–15)</h6>
                                    <Row>
                                    <Col md={6}>
                                        <div className="fw-bold mb-1">Do</div>
                                        <ul className="mb-0">
                                        {(ddData.phases?.middle?.do || []).map((d, i) => (
                                            <li key={i}>
                                            {d.text}
                                            {d.evidence && <div className="small text-muted">{d.evidence}</div>}
                                            </li>
                                        ))}
                                        {!(ddData.phases?.middle?.do || []).length && <li className="text-muted">—</li>}
                                        </ul>
                                    </Col>
                                    <Col md={6}>
                                        <div className="fw-bold mb-1">Don’t</div>
                                        <ul className="mb-0">
                                        {(ddData.phases?.middle?.dont || []).map((d, i) => (
                                            <li key={i}>
                                            {d.text}
                                            {d.evidence && <div className="small text-muted">{d.evidence}</div>}
                                            </li>
                                        ))}
                                        {!(ddData.phases?.middle?.dont || []).length && <li className="text-muted">—</li>}
                                        </ul>
                                    </Col>
                                    </Row>
                                </Card.Body>
                            ...
                            <Col md={4}>
                                <Card bg={cardVariant} text={isDarkMode ? "light" : "dark"} className="h-100">
                                <Card.Body>
                                    <h6 className="fw-bold">Death (16–20)</h6>
                                    <Row>
                                    <Col md={6}>
                                        <div className="fw-bold mb-1">Do</div>
                                        <ul className="mb-0">
                                        {(ddData.phases?.death?.do || []).map((d, i) => (
                                            <li key={i}>
                                            {d.text}
                                            {d.evidence && <div className="small text-muted">{d.evidence}</div>}
                                            </li>
                                        ))}
                                        {!(ddData.phases?.death?.do || []).length && <li className="text-muted">—</li>}
                                        </ul>
                                    </Col>
                                    <Col md={6}>
                                        <div className="fw-bold mb-1">Don’t</div>
                                        <ul className="mb-0">
                                        {(ddData.phases?.death?.dont || []).map((d, i) => (
                                            <li key={i}>
                                            {d.text}
                                            {d.evidence && <div className="small text-muted">{d.evidence}</div>}
                                            </li>
                                        ))}
                                        {!(ddData.phases?.death?.dont || []).length && <li className="text-muted">—</li>}
                                        </ul>
                                    </Col>
                                    </Row>
                                </Card.Body>
                            ...
                            {/* Debug */}
                            {process.env.NODE_ENV !== "production" && ddData && (
                            <details className="mt-3">
                                <summary>Debug: Raw response</summary>
                                <pre className="small mt-2">{JSON.stringify(ddData, null, 2)}</pre>
                            </details>
                            )}
                        </>
                        )}
